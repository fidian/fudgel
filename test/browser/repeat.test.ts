...
describe('repeat', () => {
    ...
    it('toggles based on an internal value', async () => {
        ...
        await expectText('#item0', 'Item 0');
        await expectText('#item1', 'Item 1');
        await expectText('#item2', 'Item 2');
        await click('button');
        await expectCount('test-element div', 5);
        // Growing appends; the new items come after the old ones. Indexes start at 0.
        await expectValue(() => $$('test-element div').map(d => d.textContent).join('|'), 'Item 0|Item 1|Item 2|Item 3|Item 4');
        await expectText('#item0', 'Item 0');
        await expectText('#item1', 'Item 1');
        await expectText('#item2', 'Item 2');
