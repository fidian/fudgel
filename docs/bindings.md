...
The bindings don't have to be simple property names. JavaScript syntax can perform calculations or call methods on the controller, such as a method that formats a date. A binding runs again when a top-level identifier in its expression is assigned, so pass a method the values it depends on: `{{openBrace}} format(date) {{closeBrace}}` updates when `date` is assigned, while `{{openBrace}} formatDate() {{closeBrace}}` runs once and never again. You are not allowed to use operators that change state, such as assignment (`=`), increment (`++`), or decrement (`--`). Learn more about [supported expressions](expressions.html) and [reactivity](reactivity.html).
